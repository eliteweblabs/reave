...
Same module-toggle UI as `/demo-loader`, then a Railway variable plan that prefers **shared project variables** and **reference variables** over pasted URLs. Tiles only switch on when the module is **deployed** and enabled on production Reave — requested or in-development cards stay visible but cannot be included.
...
## Source of truth is Railway shared variables

Identity and the Resend key live once as **project shared** variables. Siblings **reference** `${{ shared.KEY }}` — do not paste the same value again. Change company name or From address later under Railway → Shared Variables.

| Shared | Services read |
|--------|----------------|
| `COMPANY_NAME` | reave `COMPANY_NAME` / `EMAIL_FROM_NAME`; Cal.com `EMAIL_FROM_NAME` / `NEXT_PUBLIC_APP_NAME` / `NEXT_PUBLIC_COMPANY_NAME`; Crater `MAIL_FROM_NAME` |
| `EMAIL_FROM` | reave `EMAIL_FROM` / `RESEND_FROM`; Cal.com `EMAIL_FROM` / `NEXT_PUBLIC_SUPPORT_MAIL_ADDRESS`; Crater `MAIL_FROM_ADDRESS` |
| `PUBLIC_SITE_DOMAIN` | reave `PUBLIC_SITE_DOMAIN` / `COMPANY_DOMAIN` |
| `RESEND_API_KEY` | reave `RESEND_API_KEY`; Cal.com `RESEND_API_KEY` + `EMAIL_SERVER_PASSWORD`; Crater `MAIL_PASSWORD` |

Still on `reave` (not shared):
...
Prefer a **bare** verified address in shared `EMAIL_FROM` (`noreply@inbound.example.com`). Cal.com’s `EMAIL_FROM` is a From address, not a display-name header. The apex is a Railway CNAME, so sending uses the inbound subdomain Apply provisions.

## Identity fields

The first step writes identity onto Railway **shared** variables (and `reave` aliases that reference them):

| Field | Variable | Default |
|-------|----------|---------|
| Install slug | `INSTALL_CONFIG` / `CALCOM_USERNAME` | `demo` |
| Site domain | shared `PUBLIC_SITE_DOMAIN` (`COMPANY_DOMAIN` aliases it) | (empty) |
| Post name | `POST_ALIAS` | `project` |
| Company name | shared `COMPANY_NAME` (also `EMAIL_FROM_NAME` everywhere) | (empty) |
...
- **Derives** shared `EMAIL_FROM` as `noreply@inbound.{apex}` (the inbound domain Apply already adds in Resend) and shared `COMPANY_NAME` from the company name field. Service-level `RESEND_FROM` / `EMAIL_FROM_NAME` are `${{ shared.* }}` refs.
