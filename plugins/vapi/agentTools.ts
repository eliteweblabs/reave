...
async function handle_sync_vapi_assistant(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
  if (!hasFeature('vapi')) {
...
export const vapiModule: AgentToolModule = {
  id: 'vapi',
  enabled: (ctx) => hasFeature('vapi'),
  ...
  handlers: {
    'sync_vapi_assistant': handle_sync_vapi_assistant,
  },
};
