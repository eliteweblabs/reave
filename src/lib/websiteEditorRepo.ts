 ...
 * cannot write to the reave.app repo (eliteweblabs/reave) or any other repo.
...
/** Repos a client install must never edit — the reave.app itself. */
...
export function lockedWebsiteEditorRepo(opts: {
  ...
  if (isProtectedAppRepo(site)) {
    return {
      ok: false,
      error:
        'websiteRepo cannot be the reave.app. Point it at this install’s dedicated front-end repo.',
    };
  }
  if (requested && requested.toLowerCase() !== site.toLowerCase()) {
    return {
      ok: false,
      error: `This install can only edit ${site}. The reave.app and other repos are not writable from here.`,
    };
  }
