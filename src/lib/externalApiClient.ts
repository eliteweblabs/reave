/**
 * Shared fetch client for bootstrap satellite APIs (materials-api, inventory-api, …).
 */
...
export type ExternalApiClient = {
  ...
  apiFetch: <T>(path: string, init: { method: string; body?: unknown }) => Promise<ExternalApiResult<T>>;
};
...
export function createExternalApiClient(opts: {
  ...
  async function apiFetch<T>(
    ...
    const base = baseUrl();
    if (!base) return { ok: false, error: opts.notConfiguredError };
