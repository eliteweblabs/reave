...
export function extractMonetaryAmountFromText(text: string): number | null {
  ...
  type Scored = { amount: number; score: number };
  const scored: Scored[] = [];

  const add = (amount: number | null, score: number) => {
    if (amount != null && amount > 0) scored.push({ amount, score });
  };

  const contextual = [
    /(?:total|amount paid|paid|payment|charge(?:d)?|subtotal|balance due|you paid|grand total|order total|invoice(?:\s+(?:total|amount))?)[^$\d]{0,36}\$?\s*([\d,]+(?:\.\d{2})?)/gi,
    /\$\s*([\d,]+\.\d{2})\s*(?:\b(?:total|paid|usd)\b)/gi,
    /(?:receipt|transaction|purchase)[^$\d]{0,36}\$?\s*([\d,]+(?:\.\d{2})?)/gi,
  ];
  for (const re of contextual) {
    let match: RegExpExecArray | null;
    while ((match = re.exec(t)) !== null) {
      add(parseDollarAmount(match[1]), 3);
    }
  }
  ...
  while ((match = dollarCents.exec(t)) !== null) {
    add(parseDollarAmount(match[1]), 2);
  }
