...
export function resolveCompanyBrandColors(
  ...
  const primary = normalizeBrandColorHex(primaryRaw) ?? DEFAULT_SITE_BRAND_PRIMARY;
  const secondary = normalizeBrandColorHex(secondaryRaw) ?? DEFAULT_SITE_BRAND_SECONDARY;
...
export function companyBrandCssVars(primaryRaw?: string | null, secondaryRaw?: string | null): Record<string, string> {
  ...
  vars['--brand-gradient-shadow'] =
    `0 2px 16px rgba(${colors.secondaryRgb}, 0.35), 0 0 0 1px rgba(255, 255, 255, 0.4)`;
