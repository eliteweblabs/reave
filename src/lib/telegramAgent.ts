...
export async function runTelegramKnowledgeAgent(opts: {
  ...
  if (isContactApiConfigured()) {
    sysParts.push(
      'Contacts: full CRUD is available — create_contact, list_contacts, resolve_contact, update_contact, delete_contact. Use update_contact to change a client\'s name, email, phone, company, or notes. Use delete_contact only when explicitly asked — requires uid (no fuzzy delete). If the contact has linked jobs or invoices, warn the user and pass force:true to confirm.',
