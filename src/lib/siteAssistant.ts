...
function buildSystemPrompt(ctx: SiteAssistantContext): string {
  ...
  const lines: string[] = [
    ...
    'Useful public pages when relevant: /demo (interactive walkthrough and live sandbox), /about (team and story), /platform (tech stack and deployment), /features (full platform feature tour), /pricing (installation tiers), /modules (optional industry add-ons), /deck (scroll-through product demo), /schedule (book a call), /#contact (contact section on homepage).',
  ];
