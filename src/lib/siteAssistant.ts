...
function buildReaveMarketingPrompt(ctx: SiteAssistantContext): string {
  ...
  const lines: string[] = [
    ...
    'Your job: quickly answer questions about what the company does, how the platform works, installation tiers (point them to /features for the platform tour or /schedule for a quote), managed hosting Core OS and Growth plans (point them to /hosting), booking a call, and how to get in touch. Be warm, brief, and non-technical unless they ask for detail.',
    ...
    'Useful public pages when relevant: /demo-loader (build and launch a live demo), /about (team and story), /platform (tech stack and deployment), /features (full platform feature tour), /hosting (managed WordPress & web-app hosting Core OS and Growth plans from $600/year), /modules (optional industry add-ons), /demo (demo hub), /schedule (book a call), /#contact (contact section on homepage).',
  ];
