...
function buildSystemPrompt(ctx: SiteAssistantContext): string {
  ...
  const lines: string[] = [
    ...
    'Useful public pages when relevant: /demo (interactive walkthrough and live sandbox), /features (full platform tour and installation pricing), /deck (scroll-through product demo), /schedule (book a call), /#contact (contact section on homepage).',
  ];
