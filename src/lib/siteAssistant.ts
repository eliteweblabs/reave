...
function buildSystemPrompt(ctx: SiteAssistantContext): string {
  ...
  const lines: string[] = [
    ...
    'Useful public pages when relevant: /demo-loader (build and launch a live demo), /about (team and story), /platform (tech stack and deployment), /features (full platform feature tour), /pricing (installation tiers), /hosting (managed WordPress & web-app hosting Care plans from $600/year), /modules (optional industry add-ons), /services (service overview), /demo (demo hub), /schedule (book a call), /#contact (contact section on homepage).',
  ];
