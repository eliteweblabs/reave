...
function buildSystemPrompt(ctx: PortalAssistantContext): string {
  ...
  if (dataEntries.length) {
    ...
    lines.push(
      'When asked about a specific account, match by label or URL and share the username and link if available. For passwords, always direct the client to open the Vault tab on this page and tap the reveal icon — never disclose passwords in chat. If nothing matches, say plainly it is not on file.',
    );
  } else {
    lines.push(
