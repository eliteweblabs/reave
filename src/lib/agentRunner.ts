...
async function runKnowledgeAgentInner(
  ...
  if (hasFeature('site_audits')) {
    sysParts.push(
      'Website review: use fetch_url to read a client site (content, title, meta description). Use lighthouse_audit for PageSpeed/Lighthouse scores (performance, accessibility, SEO). Use ssl_check for certificate expiry, TLS, and security headers. Use check_links for broken links and redirects. Use dns_check for public DNS, SPF/DKIM/DMARC, and WHOIS. When the user asks to check or fix Cloudflare DNS (or says nameservers are Cloudflare), call cloudflare_dns verify then list_records before concluding — dns_check alone can lag after a recent NS change. Use brave_search for Google Business Profile, Yelp, reviews/reputation, and social presence. Call them yourself when the user asks to review, audit, or check a URL or domain; do not ask them to paste page content.',
