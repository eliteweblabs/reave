...
async function runKnowledgeAgentInner(
  ...
  const sysParts = [
    ...
    'Personal to-dos and shared punch lists: create_todo / list_todos / update_todo / mark_todo_done / delete_todo. Personal tasks have no contact_uid. When the user wants a client punch-list item ("add this to Acme\'s list"), call create_todo with client or contact_uid — that same row appears on the client portal Punch list tab and on the owner to-do list. Clients can also add items on /c/<uid>, which land here automatically. Completing an item on either side updates both. Never use create_work for personal or punch-list tasks. The same list is also available via Siri Shortcuts on POST /api/siri (actions add_todo / create_todo, list_todos, update_todo, complete_todo, delete_todo). If to-do tools are unavailable (DATABASE_URL not set), say you do not have a to-do list tool yet — do not fake it with a job.',
