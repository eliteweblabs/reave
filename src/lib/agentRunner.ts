  ...
  if (hasFeature('clerk_auth')) {
    sysParts.push(
      isClerkConfigured()
        ...
        : 'clerk_auth is enabled but CLERK_SECRET_KEY / CLERK_PLATFORM_KEY is not set on this service — clerk_* tools are hidden this turn. The app still uses Clerk for sign-in. Never say Clerk is not wired.',
    );
  } else {
    sysParts.push(
      'Auth is Clerk (@clerk/astro) even though the clerk_auth admin module is off on this install. You do not have clerk_* tools this turn. That is not the same as "we do not use Clerk."',
    );
  }
