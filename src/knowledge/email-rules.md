...
- **Rule priority:** Keyword rules are **sequential** — first enabled match wins, later rules are skipped. Sort order is the priority list (OTP/auth pinned first). Sender-specific silent rules (`from` + DELETE / notify:false) are inserted just after OTP/auth so they beat broad alert catch-alls. A matched silent rule hard-stops agent-first AI — Google “Security alert” junk cannot be re-opened as a dashboard alert. Each rule can also list **Except (NOT)** phrases — if any appear in the searched fields, that rule does not match. **Notify** is per-rule: **Push** and/or **Dashboard**, plus optional alert buttons (View, Archive, Delete, Copy code, Activate, Explain, Expense). **Else → Agent:** when nothing matches, the agent drafts a full rule (same form as Rules) for you to accept — not an admin-configured step. **Admin → Rules → Flow** is the working system (try an email, drag priority, playback); List is the classic editor.
