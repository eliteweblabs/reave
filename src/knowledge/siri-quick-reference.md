...
**Optional**: `payment_date` (`YYYY-MM-DD`), `notes`, `invoice_id`. Omitted `payment_mode` defaults to `OTHER` (Crater requires a mode). Customer aliases: `customer`, `client`, `name`. Mode aliases: `mode`, `method` (`cash`, `check`/`cheque`, `card`/`credit card`, `ach`/`bank transfer`, `other`). Amount accepts `$250`, `100 bucks`, and `100 dollars` — quote the Shortcuts variable (`"amount": "Amount"`).
