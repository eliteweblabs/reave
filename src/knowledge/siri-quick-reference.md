...
**Optional**: `payment_date` (`YYYY-MM-DD`), `notes`, `invoice_id`. `payment_mode` is any Crater payment mode name (Apple Pay, Venmo, Zelle, Stripe, Cash, Bank Transfer, …). Customer aliases: `customer`, `client`, `name`. Mode aliases: `mode`, `method`. Amount accepts `$250`, `100 bucks`, and `100 dollars` — quote the Shortcuts variable (`"amount": "Amount"`).
