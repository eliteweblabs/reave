...
export async function GET(context: APIContext): Promise<Response> {
  ...
  return jsonResponse({ ok: true, enabled: isNewsletterEnabled(), automations });
}
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  if (!id || !getAutomationDef(id)) return jsonResponse({ ok: false, error: 'Unknown automation id' }, 400);
  ...
  if (body.delayMinutes !== undefined) {
    ...
    if (!Number.isFinite(n) || n < 0) return jsonResponse({ ok: false, error: 'delayMinutes must be >= 0' }, 400);
    override.delayMinutes = Math.round(n);
  }
  if (!Object.keys(override).length) return jsonResponse({ ok: false, error: 'Nothing to update' }, 400);

  const ok = await setAutomationOverride(id, override);
  if (!ok) return jsonResponse({ ok: false, error: 'Failed to save' }, 500);

  const overrides = await getAutomationOverrides();
  const def = getAutomationDef(id)!;
  return jsonResponse({ ok: true, automation: mergeAutomation(def, overrides[id]) });
}
