...
export async function POST(context: APIContext): Promise<Response> {
  if (!serverEnv('KAP_UPLOAD_KEY')?.trim()) {
    return jsonResponse({ ok: false, error: 'KAP_UPLOAD_KEY is not configured on this service' }, 503);
  }
  ...
  if (!rate.ok) {
    return jsonResponse({ ok: false, error: 'Too many uploads. Please try again later.' }, 429);
  }
