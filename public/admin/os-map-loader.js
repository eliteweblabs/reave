...
} from './rules-panel.js?v=20260828e';
import {
  initNewsletterPanel,
  loadNewsletterTab,
...
const NAV_ICON_PATHS = {
  ...
  /* IOS_ICONS.home — keep in sync with public/admin/admin-ui.js */
  home:
...
const MEETING_NOTICE_EXPIRE_HOLD_MS = 2200;
...
function meetingNoticeExpireAtMs(item) {
  const start = meetingNoticeStartMs(item);
  if (!Number.isFinite(start)) return NaN;
  return start + MEETING_NOTICE_EXPIRE_HOLD_MS;
}

function isMeetingNoticePastHold(item) {
...
function reviewAlertItemFromElement(alert) {
  const alertId = alert.getAttribute('data-review-alert-id') || '';
  const emailId = alert.getAttribute('data-review-email-id') || '';
  const engagementId = alert.getAttribute('data-review-engagement-id') || '';
  const commentId = alert.getAttribute('data-review-comment-id') || '';
  const tag = alert.getAttribute('data-review-alert-tag') || '';
  if (!alertId && !emailId && !engagementId && !commentId) return null;
  return {
    ...(alertId ? { alertId } : {}),
    ...(emailId ? { emailId } : {}),
...
function fadeOutReviewAlert(alert, onDone) {
  ...
  window.setTimeout(() => {
    ...
    if (alert.isConnected) {
      const wrap = alert.closest('.dash-review-alerts');
      alert.remove();
...
async function dismissExpiredMeetingNotice(item) {
  if (!item) return;
  ...
  try {
    ...
    console.warn('[meeting-notice] auto-dismiss failed', e);
  }
}
...
function fadeOutExpiredMeetingAlert(alert) {
  if (alert.dataset.meetingFading === '1') return;
  alert.dataset.meetingFading = '1';
  const item = reviewAlertItemFromElement(alert);
  fadeOutReviewAlert(alert, () => void dismissExpiredMeetingNotice(item));
}
...
function syncMeetingNoticeExpiry() {
  ...
  alerts.forEach((alert) => {
    ...
    const expireAt = start + MEETING_NOTICE_EXPIRE_HOLD_MS;
    const id = alert.getAttribute('data-meeting-notice-id') || alert;

    if (expireAt <= now) {
      fadeOutExpiredMeetingAlert(alert);
      return;
    }

    if (meetingExpiredHoldTimers.has(id)) return;
    const waitMs = expireAt - now;
    const timer = window.setTimeout(() => {
      meetingExpiredHoldTimers.delete(id);
      if (!alert.isConnected || !isDashboardViewActive()) return;
      fadeOutExpiredMeetingAlert(alert);
    }, waitMs);
    meetingExpiredHoldTimers.set(id, timer);
  });
}
...
function siteHealthSignalState(key, health, card, fleet, opts = {}) {
  ...
  if (key === 'robots') {
    ...
    if (codes.has('robots_blocked')) return 'crit';
    if (codes.has('robots_missing')) return 'warn';
    return 'ok';
  }
  if (key === 'gsc') {
    if (!health) return 'unknown';
    if (codes.has('gsc_missing')) return 'crit';
    if (codes.has('gsc_unconnected')) return 'warn';
    if (fleet?.googleConnected === false) return 'warn';
    return 'ok';
  }
  if (key === 'analytics') {
    if (card.analytics) {
      return card.analytics.registered ? 'ok' : 'warn';
    }
    if (opts.analyticsLoading) return 'unknown';
    if (opts.analyticsLive) return 'warn';
    return 'unknown';
  }
  return 'unknown';
}

function siteHealthSignalTitle(key, state, health, card, opts = {}) {
  const codes = siteHealthIssueCodes(health);
  if (key === 'robots') {
    if (state === 'crit') return 'robots.txt blocks crawlers';
    if (state === 'warn') return 'No robots.txt';
    if (state === 'ok') return 'robots.txt OK';
    return 'robots.txt — checking…';
  }
  if (key === 'gsc') {
    if (state === 'crit') return 'Not in Search Console';
    if (state === 'warn') {
      if (codes.has('gsc_unconnected')) return 'Search Console not connected';
      return 'Search Console not connected';
    }
    if (state === 'ok') return 'In Search Console';
    return 'Search Console — checking…';
  }
  if (key === 'analytics') {
    if (state === 'warn') return 'Analytics not wired';
    if (state === 'ok') return 'Analytics wired';
    if (opts.analyticsLoading) return 'Analytics — loading…';
    return 'Analytics';
  }
  return '';
}
...
function buildReviewAlertBanners(notifications) {
  ...
  for (const item of filterPendingDismissNotifications(notifications)) {
    if (isExpiringMeetingNotice(item) && isMeetingNoticePastHold(item)) {
      void dismissExpiredMeetingNotice(item);
      continue;
    }
        ...
        `<form id="company-form" class="prof-form">` +
          ...
          profSection(
            ...
            `<div class="prof-font-preview" aria-hidden="true">` +
              ...
              `<p class="prof-font-preview-primary">Run your whole business</p>` +
...
function getEmailLabBodyFrameDoc() {
  ...
  try {
    ...
    if (!doc || iframeDocLooksEmpty(doc)) return null;
    return doc;
  } catch {
    return null;
  }
...
function labDocFromEventTarget(ev) {
  const t = ev?.currentTarget;
  if (t?.nodeType === 9) return t;
  if (t?.ownerDocument) return t.ownerDocument;
  return document;
}

function labSelectionDocHasText(doc) {
  const sel = selectionFromContext(doc);
  return Boolean(sel && !sel.isCollapsed && String(sel.toString() || '').trim());
}

function captureLabSelectionFromSource() {
  if (!emailState.labMode || labSelecting) return false;
  const detail = emailState.labDetail;
  if (!detail) return false;

  const tryDoc = (doc, field) => {
    if (!doc || !labSelectionDocHasText(doc)) return false;
    return captureLabWindowSelection(doc, field, detail);
  };

  const frameDoc = getEmailLabBodyFrameDoc();
  const source = labSelectionSource;
  labSelectionSource = null;

  if (source === frameDoc) return tryDoc(frameDoc, 'body');
  if (source && source !== document) return tryDoc(source, null);
  if (tryDoc(document, null)) return true;
  return tryDoc(frameDoc, 'body');
}
...
function appendEmailLabChip(list, { text, field, onRemove }) {
  const li = document.createElement('li');
...
function markLabSelecting(ev) {
  ...
  if (!fromIframe && el?.closest('.em-detail-body-frame, .em-detail-body-html')) return;
...
function commitLabSelectionNow() {
  ...
  const frameDoc = getEmailLabBodyFrameDoc();
  const hadSelection =
    labSelectionDocHasText(document) || (frameDoc ? labSelectionDocHasText(frameDoc) : false);
  captureLabSelectionFromSource();
  if (hadSelection) clearEmailLabSelections();
}
...
function onLabKeyUp(ev) {
  ...
  if ((key === 'a' || key === 'A') && (ev.ctrlKey || ev.metaKey)) {
    finishLabSelecting();
  }
}
