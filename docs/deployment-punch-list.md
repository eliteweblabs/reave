...
| Email inbox (always on) | Shared `RESEND_API_KEY` + `EMAIL_FROM` (`noreply@inbound.{apex}`) and `RESEND_WEBHOOK_SECRET` on `reave` + Resend webhook → `/api/email/inbound`. Cal.com / Crater read `${{ shared.EMAIL_FROM }}` / `${{ shared.COMPANY_NAME }}` / `${{ shared.RESEND_API_KEY }}` — do not leave Cal.com `EMAIL_FROM` unset (it falls back to sendmail). |
