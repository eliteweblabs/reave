...
Stack logos: curated `SALES_SHEET_STACK` (Astro, Railway, Supabase, Clerk,
Resend, Anthropic, Telnyx, GitHub, Cloudflare, Cal.com, Plausible) — black
marks only, no labels, no “builds with” line, pinned to the bottom of the
back-cover column.
...
The sheet uses `/reave-bg-pattern.svg` at the original tile scale, **15%**
black. Each column’s radial wash fills the panel — white in the middle,
darker at the edges.
