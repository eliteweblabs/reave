...
assert.ok(FEATURE_IDS.includes('social_lead_scanner'));
...
assert.equal(FEATURE_LABELS.social_lead_scanner, 'Agentic Social Lead Scanner');
assert.ok(featureShowsDashboard('social_inbox'));
assert.ok(featureShowsDashboard('social_lead_scanner'));
assert.equal(FEATURE_DASHBOARD.social_inbox?.icon, 'share');
assert.equal(FEATURE_DASHBOARD.social_lead_scanner?.icon, 'search');
const dashCards = dashboardCardsForFeatures(['social_inbox', 'social_lead_scanner', 'online_reviews', 'email_marketing', 'website']);
assert.ok(dashCards.some((c) => c.id === 'social_inbox' && c.title === FEATURE_LABELS.social_inbox));
assert.ok(dashCards.some((c) => c.id === 'social_lead_scanner' && c.title === FEATURE_LABELS.social_lead_scanner));
