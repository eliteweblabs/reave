 ...
 * writes WebP sources to src/assets/images/portfolio/ for Astro `<Picture>`.
 *
...
function pickInputs(files) {
  ...
  for (const file of files) {
    ...
    const base = file.replace(/\.(png|jpe?g|webp)$/i, '').toLowerCase();
    const ext = path.extname(file).toLowerCase();
    const rank = ext === '.jpg' || ext === '.jpeg' ? 0 : ext === '.webp' ? 1 : 2;
...
async function optimizeOne(inputFile) {
  ...
  const base = inputFile.replace(/\.(png|jpe?g|webp)$/i, '');
  ...
  // Drop stale JPEG/PNG siblings so imports stay unambiguous.
  for (const stale of [`${base}.jpg`, `${base}.jpeg`, `${base}.png`]) {
...
async function main() {
  ...
  for (const file of inputs) {
    ...
    totalOut += fs.statSync(
      path.join(OUTPUT_DIR, file.replace(/\.(png|jpe?g|webp)$/i, '.webp')),
    ).size;
  }
