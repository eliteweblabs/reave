import type { ComplianceInput, ComplianceItem, ComplianceRule, ComplianceTimeline } from './types.js';

const DEFAULT_DISCLAIMER =
  'Informational only — not legal advice. Requirements vary by municipality; verify with local code enforcement and licensed professionals.';
...
function ruleAppliesToState(rule: ComplianceRule, state: string): boolean {
  if (!rule.states?.length) return true;
  return rule.states.includes(state);
}

function evaluateRule(rule: ComplianceRule, input: ComplianceInput, age: number | null): ComplianceItem | null {
  const state = normalizeState(input.state);
  if (!ruleAppliesToState(rule, state)) return null;
  if (rule.requiresSeptic && !input.hasSeptic) return null;
  if (rule.requiresRental && !input.isRental) return null;
...
export function buildComplianceTimeline(input: ComplianceInput): ComplianceTimeline {
  ...
  const dueSoon: ComplianceItem[] = [];
  const upcoming: ComplianceItem[] = [];
  const informational: ComplianceItem[] = [];
  ...
  const sort = (a: ComplianceItem, b: ComplianceItem) => urgencyRank[a.urgency] - urgencyRank[b.urgency];
