...
function rowVal(row: Record<string, unknown>, field?: string): string {
  if (!field) return '';
  const v = row[field];
  return v == null ? '' : String(v).trim();
}

function mapRow(row: Record<string, unknown>, feed: ViolationFeedConfig, source: string): CodeViolation {
  const f = feed.fields;
...
function addressMatches(input: LookupInput, row: Record<string, unknown>, feed: ViolationFeedConfig): boolean {
  const f = feed.fields;
  ...
  if (f.fullAddress) {
    ...
    if (full && (full.includes(needle) || needle.includes(full.split(',')[0] ?? ''))) return true;
  }

  const rowStreet = normalizeStreetToken(rowVal(row, f.street));
  const rowNum = rowVal(row, f.streetNumber);
  if (!rowStreet && !rowNum) return false;
  if (inputNum && rowNum && inputNum !== rowNum) return false;
...
export async function queryCkanFeed(
  feed: Extract<ViolationFeedConfig, { type: 'ckan' }>,
  input: LookupInput,
  ...
  if (feed.fields.street && parsed.streetName) {
    ...
    if (streetToken) filters[feed.fields.street] = streetToken;
  }
...
export async function querySocrataFeed(
  feed: Extract<ViolationFeedConfig, { type: 'socrata' }>,
  input: LookupInput,
  appToken?: string,
): Promise<CodeViolation[]> {
  const parsed = parseStreetAddress(input.address);
  ...
  if (feed.fields.fullAddress && input.address.trim()) {
    ...
    if (token) url.searchParams.set('$where', `upper(${feed.fields.fullAddress}) like '%${token.toUpperCase()}%'`);
  } else if (feed.fields.street && parsed.streetName) {
    const token = parsed.streetName.split(/\s+/)[0]?.toUpperCase();
    if (token) url.searchParams.set('$where', `upper(${feed.fields.street}) like '%${token}%'`);
  }
...
export async function queryFeed(
  feed: ViolationFeedConfig,
  input: LookupInput,
  appToken?: string,
): Promise<CodeViolation[]> {
  if (feed.type === 'ckan') return queryCkanFeed(feed, input);
