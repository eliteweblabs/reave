import { lookupComps, lookupProperty } from './providers/index.js';
...
function parseFloorArg(floor?: number | string, floorLabel?: string): number | string | undefined {
  if (floor != null) return floor;
  if (!floorLabel) return undefined;
  const lower = floorLabel.toLowerCase();
  const ordinals: Record<string, number | string> = {
    first: 1,
    '1st': 1,
    second: 2,
    '2nd': 2,
 ...
 */
export async function getFloorArea(input: FloorAreaQuery): Promise<FloorAreaResult> {
  ...
  if (property.floorAreas?.length && floor != null) {
    const match = property.floorAreas.find((f) => {
      if (typeof floor === 'number') return f.floor === floor;
