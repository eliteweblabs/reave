...
export function normalizeTradeSlugs(input: string[]): TradeSlug[] {
  const out: TradeSlug[] = [];
  for (const raw of input) {
    const slug = raw.trim().toLowerCase().replace(/\s+/g, '_') as TradeSlug;
    if (TRADE_BY_SLUG.has(slug)) out.push(slug);
  }
  return [...new Set(out)];
}
