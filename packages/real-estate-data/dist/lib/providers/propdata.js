...
async function propDataFetch(path, params) {
...
export const propdataProvider = {
    ...
    async lookupComps(input) {
        const params = { limit: String(input.limit ?? 10) };
        if (input.parcelId) {
            params.parcel = input.countyFips ? `${input.countyFips}-${input.parcelId}` : input.parcelId;
        }
        ...
        if (input.bedrooms != null)
            params.beds = String(input.bedrooms);
        let res;
        try {
        ...
        const data = (await res.json());
        if (data.error)
            return { ok: false, error: data.error, code: 'PROVIDER_ERROR' };
        const comps = (data.results ?? []).map((row) => ({
