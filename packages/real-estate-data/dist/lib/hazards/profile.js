...
function floodLevelFromZone(zone) {
    if (!zone)
        return 'none';
    const z = zone.toUpperCase();
    if (z.startsWith('V') || z.startsWith('VE'))
        return 'severe';
    if (z.startsWith('A') || z.includes('AE') || z.includes('AH'))
        return 'high';
    if (z.startsWith('X') && z.includes('SHADED'))
        return 'moderate';
    if (z.startsWith('X'))
        return 'low';
    if (z.startsWith('D'))
...
export function buildHazardProfile(property, opts) {
    const state = (property.state ?? '').toUpperCase();
    const floodZone = property.floodZone ?? null;
    let wildfireLevel = opts?.wildfireLevel ?? 'none';
    ...
    return {
        ...
        fireDamage: {
            ...
            note: 'Insurance fire claims are not public. Rebuild permits or disaster declarations may indicate past fire damage.',
        },
        storm: {
