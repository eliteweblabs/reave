...
function createHasFeature(opt?: RealEstateDataModuleOptions): HasFeature {
  return (
    opt?.hasFeature ??
    ((id: string) => {
      if (id !== DEFAULT_FEATURE_ID) return false;
...
async function handle_lookup_property(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
  const result = await lookupProperty({
    ...
    countyFips:
      args.county_fips != null ? String(args.county_fips) : args.countyFips != null ? String(args.countyFips) : undefined,
  });
...
async function handle_get_property_floor_area(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
...
async function handle_get_property_year_built(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
  const result = await lookupProperty({
    address: args.address != null ? String(args.address) : undefined,
    city: args.city != null ? String(args.city) : undefined,
    state: args.state != null ? String(args.state) : undefined,
  ...
  if (!property) return JSON.stringify({ error: 'No property matched that address', code: 'NOT_FOUND' });
...
async function handle_search_property_comps(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
...
function addressArgs(args: Record<string, unknown>) {
  return {
    address: args.address != null ? String(args.address) : undefined,
    city: args.city != null ? String(args.city) : undefined,
    state: args.state != null ? String(args.state) : undefined,
...
async function handle_get_property_compliance_timeline(
...
async function handle_get_property_hazard_profile(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
  const lookup = await lookupProperty(addressArgs(args));
  if (!lookup.ok) return JSON.stringify({ error: lookup.error, code: lookup.code });
  const property = lookup.properties[0];
  if (!property) return JSON.stringify({ error: 'No property matched', code: 'NOT_FOUND' });
...
async function handle_run_lead_scan(args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
...
async function handle_real_estate_data_status(_args: Record<string, unknown>, _ctx: ToolContext): Promise<string> {
      ...
      return [
        ...
        },
        {
          ...
          function: {
            ...
            description:
              'Get square footage for a specific floor of a property (e.g. "2nd floor"). Most county assessor sources only provide total building sqft — when per-floor data is missing, returns an estimate split evenly across stories with a clear note. Prefer this over guessing from total sqft.',
            parameters: {
              type: 'object',
              properties: {
                address: { type: 'string' },
        ...
        },
        {
          ...
          function: {
            ...
            description:
              'When a user asks when a property was built or its construction year. Returns year built from county assessor / parcel records when available.',
            parameters: {
              type: 'object',
              properties: {
                address: { type: 'string' },
        ...
        },
        {
          ...
          function: {
            ...
            description:
              'Find comparable recent sales near a property — useful for agents, developers, and attorneys evaluating market context.',
            parameters: {
              type: 'object',
              properties: {
                address: { type: 'string' },
        ...
        },
        {
          ...
          function: {
            ...
            description:
              'Chronological compliance and lifecycle items for a property — what may be overdue based on year built and state (roof, electrical panel, plumbing, HVAC, lead paint, septic, smoke/CO). Informational, not legal advice.',
            parameters: {
              type: 'object',
              properties: {
                address: { type: 'string' },
        ...
        },
        {
          ...
          function: {
            ...
            description:
              'Full Property Liability Radar — compliance timeline, hazards, violations, and trade-specific lead score. Use for homeowner reports and contractor outreach briefs.',
            parameters: {
              type: 'object',
              properties: {
                address: { type: 'string' },
        ...
        },
        {
          ...
          function: {
            name: 'run_lead_scan',
            description:
              'Scan properties within a radius (miles) of a center point for leads matching target trades. Uses configured property data provider; returns scored candidates.',
            parameters: {
              type: 'object',
              properties: {
